...
jQuery(document).ready(function($) {
  ...
  var stickyFillInit = function() {
	...
	$('#profile_in').on('mousewheel',function(e){
		if($(this).scrollTop()>=$('#profile_in img').height()-$(this).height() && e.originalEvent.wheelDelta < 0){
			e.preventDefault();
		}
	...
	$(window).scroll(function(){
		if($(window).scrollTop()>screen.height*1/3){
			$('.bar-fill').addClass('on');
		}

	});

	$('.main_visual').ready(function(){
		$('.main_visual').slick({
			slide:'div',
			dots : true,
			arrows : true
